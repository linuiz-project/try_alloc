use crate::{raw_vec::RawVec, GlobalAllocator, Result};
...
impl<T> Deque<T, GlobalAllocator> {
    ...
    pub fn with_capacity(capacity: usize) -> Result<Self> {
        RawVec::with_capacity(capacity).map(|buffer| Self {
            buffer,
            head: 0,
            tail: 0,
        })
...
impl<T, A: Allocator> Deque<T, A> {
    pub const fn new_in(allocator: A) -> Self {
        Self {
            buffer: RawVec::new_in(allocator),
            head: 0,
            tail: 0,
        }
    }

    pub fn with_capacity_in(capacity: usize, allocator: A) -> Result<Self> {
        RawVec::with_capacity_in(capacity, allocator).map(|buffer| Self {
            buffer,
            head: 0,
            tail: 0,
        })
    ...
    fn grow_one(&mut self) -> Result<()> {
        ...
        debug_assert_eq!(
            self.capacity(),
            crate::next_capacity(old_capacity.wrapping_add(1))
        );
